...
class SourceReferenceController extends Controller
{
    ...
    public function show(SourceReference $sourceReference): Response
    {
        ...
        return Inertia::render('Editor/SourceReferences/Show', [
            'sourceReference' => [
                ...$this->sourcePayload($sourceReference),
                'notes' => $sourceReference->notes,
                'editorial_schedule_run' => $sourceReference->editorialScheduleRun ? [
                    'id' => $sourceReference->editorialScheduleRun->id,
                    'status' => $sourceReference->editorialScheduleRun->status,
                    'scheduled_for' => $sourceReference->editorialScheduleRun->scheduled_for?->toDateTimeString(),
                ] : null,
            ],
            'verificationStatuses' => SourceReference::VERIFICATION_STATUSES,
            'sourceTypes' => SourceReference::SOURCE_TYPES,
    ...
    public function update(Request $request, SourceReference $sourceReference): RedirectResponse
    {
        ...
        $currentStatus = $sourceReference->verification_status;

        if ($currentStatus !== $data['verification_status'] && in_array($data['verification_status'], ['verified', 'weak', 'broken', 'rejected', 'missing'], true)) {
    ...
    public function extractFromScriptReviewItem(ScriptReviewItem $scriptReviewItem): RedirectResponse
    {
    ...
    public function createFromNewsItem(\App\Models\NewsItem $newsItem): RedirectResponse
    {
        if (! $newsItem->source_url) {
            return back()->with('error', 'No source references found');
        }

        SourceReference::query()->firstOrCreate(
            [
