...
$this->markTaskRunning('Generating AI response'); $run=BulletinPromptRun::query()->with('bulletinType.preferredAiProvider')->findOrFail($this->bulletinPromptRunId); if($run->status==='archived'||blank($run->generated_prompt)) throw new \RuntimeException('Invalid prompt run state.');
$provider=$this->aiProviderId?AiProvider::query()->active()->find($this->aiProviderId):$run->bulletinType?->preferredAiProvider; if(! $provider || ! $provider->is_active) throw new \RuntimeException('No AI provider configured for this bulletin.');
if($provider->requiresApiKey() && ! $provider->hasConfiguredApiKey()) throw new \RuntimeException('Environment key '.($provider->apiKeyEnvName() ?: 'API_KEY').' is not configured.');
...
$started=now(); $response=$manager->generateText($provider, (string)$run->generated_prompt, ['model'=>$this->model ?: $provider->default_model]);
