...
interface Provider {
    id:number; name:string; slug:string; provider_type:string; client_driver:string; base_url:string|null; api_key_env_name:string|null; default_model:string|null; organization:string|null;
...
export default function AiProviderForm({ provider, providerTypes = [] }: Props): JSX.Element {
    ...
    const form = useForm({
        ...
        client_driver: provider?.client_driver ?? 'custom',
    ...
    return <AdminLayout><Head title={isEdit ? t('Edit AI Provider') : t('Create AI Provider')} /><AdminPageHeader helpKey="admin.aiproviders.form" title={isEdit ? t('Edit AI Provider') : t('Create AI Provider')} description={t('Technical provider configuration.')} />
        <Card><CardContent className="pt-6"><form onSubmit={submit} className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-4"><div><Label>{t('Provider type')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.provider_type} onChange={(e)=>form.setData('provider_type', e.target.value)}>{providerTypes.map((item)=><option key={item} value={item}>{item}</option>)}</select>{form.data.provider_type === 'groq' ? <p className="mt-1 text-xs text-slate-500">{t('Groq uses an OpenAI-compatible endpoint and reads the key from GROQ_API_KEY.')}</p> : null}</div><div><Label>{t('AI client driver')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.client_driver} onChange={(e)=>form.setData('client_driver', e.target.value)}><option value="custom">{t('Custom client')}</option><option value="laravel_ai">{t('Laravel AI SDK')}</option></select><p className="mt-1 text-xs text-slate-500">{t("Laravel AI SDK uses Laravel 13's official AI integration layer. Existing logs and limits still apply.")}</p></div><div><Label>{t('Base URL')}</Label><Input value={form.data.base_url} onChange={(e)=>form.setData('base_url', e.target.value)} /></div><div><Label>{t('API key environment name')}</Label><Input value={form.data.api_key_env_name} onChange={(e)=>form.setData('api_key_env_name', e.target.value)} /></div></div>
