...
export default function Index({ providers }: any): JSX.Element {
  ...
  const runTest = async (provider: any, testType: 'minimal' | 'short-script' | 'grounded' | 'official-minimal-gemini') => {
    const key = `${provider.id}-${testType}`;
    setLoadingKey(key);
    setDiagProvider(provider);
    try {
      const url = testType === 'minimal' ? route('admin.ai-providers.tests.minimal', provider.id) : testType === 'short-script' ? route('admin.ai-providers.tests.short-script', provider.id) : testType === 'official-minimal-gemini' ? route('admin.ai-providers.tests.official-minimal-gemini', provider.id) : route('admin.ai-providers.tests.grounded', provider.id);
  ...
  useEffect(() => {
    const close = () => setActionMenuProviderId(null);
    const onResize = () => setActionMenuProviderId(null);
