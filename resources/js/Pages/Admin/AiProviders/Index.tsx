    ...
    return <AdminLayout><Head title={t('AI Providers')} /><AdminPageHeader helpKey="admin.aiproviders.index" title={t('AI Providers')} description={t('Providers are grouped by what they do and keep limits/cost visibility.')} actionLabel={t('Create AI Provider')} actionHref={route('admin.ai-providers.create')} />
        ...
        <Card><CardContent className='overflow-x-auto pt-6'>
            <table className='w-full min-w-[1200px] text-sm'><thead><tr className='border-b'><th className='py-2 text-left'>{t('Name')}</th><th className='text-left'>{t('Provider category')}</th><th className='text-left'>{t('Model')}</th><th className='text-left'>{t('Request usage')}</th><th className='text-left'>{t('Token costs')}</th><th className='text-left'>{t('Config')}</th><th className='text-right'>{t('Actions')}</th></tr></thead><tbody>
                {filtered.map((p: any) => <tr key={p.id} className='border-b align-top'><td className='py-3'><p className='font-semibold'>{p.name} {p.is_default && <Badge>{t('Default provider')}</Badge>} {p.is_testing && <Badge variant='outline'>{t('Mock provider is only for testing')}</Badge>}</p><div className='mt-1 flex flex-wrap gap-1'>{(p.capabilities ?? []).map((c: string) => <Badge key={c} variant='outline'>{capabilityLabel(t, c)}</Badge>)}</div></td><td className='py-3'>{categoryLabel(t, p.provider_category)}</td><td className='py-3'>{p.default_model || '-'}<div className='text-xs text-slate-500'>max={p.max_tokens ?? '-'} · temp={p.temperature ?? '-'} · timeout={p.timeout_seconds ?? '-'}</div></td><td className='py-3'>{t('Daily request limit')}: {p.daily_request_limit ?? '-'}<br />{t('Monthly request limit')}: {p.monthly_request_limit ?? '-'}<br />{t('Used today')}: {p.usage_summary?.daily_requests ?? '-'}<br />{t('Remaining today')}: {p.usage_summary?.remaining_daily_requests ?? '-'}</td><td className='py-3'>{t('Input token cost')}: {p.cost_input_per_1k_tokens ?? '-'}<br />{t('Output token cost')}: {p.cost_output_per_1k_tokens ?? '-'}<br />{t('Estimated cost')}: {p.usage_summary?.daily_estimated_cost ?? '-'}</td><td className='py-3'>{p.env_key_configured ? <Badge>{t('Environment key configured')}</Badge> : <Badge variant='outline'>{t('Environment key missing')}</Badge>}<div className='mt-1 text-xs text-slate-500'>{p.api_key_env_name || '-'}</div><div className='mt-1 text-xs text-slate-500'>{t('Rate limited until')}: {p.rate_limited_until || '-'}</div></td><td className='py-3 text-right'><div className='flex justify-end gap-2'><Button size='sm' variant={p.is_active ? 'secondary' : 'outline'} onClick={() => router.post(route('admin.ai-providers.toggle-active', p.id))}>{p.is_active ? t('Active provider') : t('Inactive provider')}</Button><Button size='sm' variant={p.is_default ? 'default' : 'outline'} onClick={() => router.post(route('admin.ai-providers.make-default', p.id))}>{t('Make default')}</Button><Button size='sm' variant='outline' onClick={() => router.post(route('admin.ai-providers.clear-rate-limit-lock', p.id))}>{t('Clear rate limit lock')}</Button><Button asChild size='sm' variant='outline'><Link href={route('admin.ai-providers.show', p.id)}>{t('Show')}</Link></Button><Button asChild size='sm' variant='outline'><Link href={route('admin.ai-providers.edit', p.id)}>{t('Edit')}</Link></Button></div></td></tr>)}
            </tbody></table>
        </CardContent></Card>
    </AdminLayout>;
}
