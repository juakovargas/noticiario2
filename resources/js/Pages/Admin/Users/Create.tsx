...
export default function UsersCreate({ roles, rolesWithPermissions, permissions, localeOptions, dateFormatOptions, timeFormatOptions }: UsersCreateProps): JSX.Element {
    ...
    const submit = (event: React.FormEvent): void => { event.preventDefault(); post(route('admin.users.store'), { forceFormData: true, preserveScroll: true }); };
    ...
    return <AdminLayout>
        ...
        <Card><CardContent className="pt-6"><form className="space-y-6" onSubmit={submit} encType="multipart/form-data">
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <div><Label htmlFor="password">{t('Password')}</Label><Input id="password" type="password" value={data.password} onChange={(e) => setData('password', e.target.value)} required />{errors.password && <p className="mt-1 text-xs text-rose-600">{errors.password}</p>}</div>
                <div><Label htmlFor="password_confirmation">{t('Confirm password')}</Label><Input id="password_confirmation" type="password" value={data.password_confirmation} onChange={(e) => setData('password_confirmation', e.target.value)} required /></div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
                <div><p className="mb-2 text-sm font-medium text-slate-700">{t('User roles')}</p><ul className="mb-3 list-disc space-y-1 pl-4 text-xs text-slate-600"><li>{t('Superadmin')}: {t('Full system access')}</li><li>{t('Admin')}: {t('Technical administration')}</li><li>{t('Editor')}: {t('Editorial workspace access')}</li><li>{t('Viewer')}: {t('Read-only panel access')}</li></ul><div className="max-h-48 space-y-2 overflow-auto rounded-md border border-slate-200 p-3">{roles.map((role) => <label key={role.id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={data.roles.includes(role.name)} onChange={() => toggleArrayValue('roles', role.name)} className="rounded border-slate-300" />{role.name}</label>)}</div></div>
            ...
            <div className="rounded-md border border-slate-200 p-3"><p className="text-sm font-semibold">{t('Effective permissions')}</p><p className="mt-1 text-xs text-slate-500">{data.roles.length ? `${t('Selected roles')}: ${data.roles.join(', ')}` : t('No permissions selected')}</p><div className="mt-2 text-sm">{effectivePermissions.length ? effectivePermissions.join(', ') : t('No permissions selected')}</div><p className="mt-3 text-sm font-medium">{t('Panel access')}</p><ul className="text-sm text-slate-700"><li>{effectivePermissions.includes('admin.access') ? t('Can access Admin panel') : t('Cannot access Admin panel')}</li><li>{effectivePermissions.includes('editor.access') ? t('Can access Editor panel') : t('Cannot access Editor panel')}</li><li>{effectivePermissions.includes('viewer.access') ? t('Can access Viewer panel') : t('Cannot access Viewer panel')}</li></ul></div>
