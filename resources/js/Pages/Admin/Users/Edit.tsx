...
interface User { id: number; name: string; email: string; is_active: boolean; roles: string[]; permissions: string[]; preferred_locale: string | null; timezone: string | null; date_format: string | null; time_format: string | null; avatar_url: string | null; }
interface UsersEditProps { user: User; roles: RolePermissionOption[]; permissions: RolePermissionOption[]; localeOptions: LocaleOption[]; dateFormatOptions: string[]; timeFormatOptions: string[]; }
interface FormData { name: string; email: string; password: string; password_confirmation: string; is_active: boolean; roles: string[]; permissions: string[]; preferred_locale: string; timezone: string; date_format: string; time_format: string; avatar: File | null; }
...
export default function UsersEdit({ user, roles, permissions, localeOptions, dateFormatOptions, timeFormatOptions }: UsersEditProps): JSX.Element {
    const { data, setData, put, processing, errors } = useForm<FormData>({ name: user.name, email: user.email, password: '', password_confirmation: '', is_active: user.is_active, roles: user.roles, permissions: user.permissions, preferred_locale: user.preferred_locale ?? '', timezone: user.timezone ?? '', date_format: user.date_format ?? 'locale_default', time_format: user.time_format ?? '24h', avatar: null });
    ...
    const submit = (event: React.FormEvent): void => { event.preventDefault(); put(route('admin.users.update', user.id)); };

    return <AdminLayout><Head title="Edit User" /><AdminPageHeader title="Edit User" description="Update profile, access, and status." />
        <Card><CardContent className="pt-6"><form className="space-y-6" onSubmit={submit} encType="multipart/form-data">
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <div><Label htmlFor="password">New password (optional)</Label><Input id="password" type="password" value={data.password} onChange={(event) => setData('password', event.target.value)} />{errors.password && <p className="mt-1 text-xs text-rose-600">{errors.password}</p>}</div>
