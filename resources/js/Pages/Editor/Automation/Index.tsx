        ...
        <table className='min-w-full text-sm'>
          ...
          <tbody>{bulletins.map((b) => <tr key={b.id} className='border-t align-top'>
            ...
            <td className='p-2'><div className='flex flex-col gap-1'>
              {b.has_schedule ? <Button size='sm' onClick={() => { if (confirm(t('This will send the prompt to the configured AI provider and may consume one request'))) router.post(route('editor.automation.schedules.run-full-pipeline', b.schedule_id)); }}>{b.next_run_status === 'overdue' ? t('Run missed pipeline now') : t('Run full pipeline now')}</Button> : null}
