...
export default function Form({ bulletinType, locations = [], categories = [], languages = [], promptProfiles = [], scriptProviders = [], editionTypes = [], coverageModes = [], promptLanguages = [], outputModes = [] }: any): JSX.Element {
    ...
    return <EditorLayout>
        ...
        <AdminPageHeader helpKey={isEdit ? "editor.bulletintypes.edit" : "editor.bulletintypes.create"} title={isEdit ? t('bulletinTypes.form.editTitle') : t('bulletinTypes.form.createTitle')} description={t('bulletinTypes.form.description')} />
        <Card><CardContent className="pt-6"><form onSubmit={submit} className="space-y-5">
            ...
            <div className="grid gap-4 md:grid-cols-2"><div><Label>{t('bulletinTypes.form.preferredProvider')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2 dark:border-slate-700 dark:bg-slate-900" value={form.data.preferred_ai_provider_id} onChange={(e)=>form.setData('preferred_ai_provider_id', e.target.value)}><option value="">{t('bulletinTypes.provider.missing')}</option>{scriptProviders.map((x:any)=><option key={x.id} value={x.id}>{x.name} · {x.default_model || t('bulletinTypes.empty.noModel')} · {t(x.supports_grounding ? 'bulletinTypes.provider.grounded' : 'bulletinTypes.provider.notGrounded')}</option>)}</select><p className="mt-1 text-xs text-slate-500">{t('bulletinTypes.form.providerHelp')}</p></div><div><Label>{t('Language')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.language_id} onChange={(e)=>form.setData('language_id', e.target.value)}><option value="">{t('Any')}</option>{languages.map((x:any)=><option key={x.id} value={x.id}>{x.name} ({x.code})</option>)}</select></div><div><Label>{t('Default prompt profile')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.default_prompt_profile_id} onChange={(e)=>form.setData('default_prompt_profile_id', e.target.value)}><option value="">{t('Any')}</option>{promptProfiles.map((x:any)=><option key={x.id} value={x.id}>{x.name}</option>)}</select></div></div>
            ...
            <div className="space-y-3 rounded-lg border p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2"><div><Label>{t('Prompt language')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.prompt_language} onChange={(e)=>form.setData('prompt_language', e.target.value)}>{promptLanguages.map((x:string)=><option key={x} value={x}>{x}</option>)}</select></div><div><Label>{t('Output mode')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.output_mode} onChange={(e)=>form.setData('output_mode', e.target.value)}>{outputModes.map((x:string)=><option key={x} value={x}>{t(x === 'plain_final_script' ? 'Simple final script' : 'Structured script advanced/debug')} {x === 'structured_script' ? `(${t('Advanced / debug')})` : ''}</option>)}</select></div></div>
                <p className="text-xs text-slate-600">{form.data.output_mode === 'plain_final_script' ? t('This mode is recommended for automation') : form.data.output_mode === 'structured_script' ? t('Structured mode is only for advanced debugging') : t('The AI response can be saved directly as a script')}</p>
