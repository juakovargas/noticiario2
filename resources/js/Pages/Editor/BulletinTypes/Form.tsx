    ...
    return <EditorLayout>
        ...
        <Card><CardContent className="pt-6"><form onSubmit={submit} className="space-y-5">
            ...
            <div className="space-y-3 rounded-lg border p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2"><div><Label>{t('Prompt language')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.prompt_language} onChange={(e)=>form.setData('prompt_language', e.target.value)}>{promptLanguages.map((x:string)=><option key={x} value={x}>{x}</option>)}</select></div><div><Label>{t('Output mode')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.output_mode} onChange={(e)=>form.setData('output_mode', e.target.value)}>{outputModes.map((x:string)=><option key={x} value={x}>{t(x === 'final_plain_script' ? 'Simple final script' : x === 'plain_script' ? 'Plain script' : 'Structured script')}</option>)}</select></div></div>
                <p className="text-xs text-slate-600">{form.data.output_mode === 'final_plain_script' ? t('This mode is recommended for automation') : form.data.output_mode === 'structured_script' ? t('Structured mode is useful for debugging') : t('The AI response can be saved directly as a script')}</p>
