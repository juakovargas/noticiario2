...
export default function Form({ bulletinType, locations = [], categories = [], languages = [], promptProfiles = [], scriptProviders = [], editionTypes = [], coverageModes = [], promptLanguages = [], outputModes = [] }: any): JSX.Element {
    ...
    const form = useForm({
        ...
        ai_provider_id: bulletinType?.ai_provider_id?.toString() ?? '',
        ...
        output_mode: bulletinType?.output_mode ?? 'final_plain_script',
    ...
    return <EditorLayout>
        ...
        <AdminPageHeader helpKey="editor.bulletintypes.index" title={isEdit ? t('Edit Bulletin Type') : t('Create Bulletin Type')} description={t('Noticiario type')} />
        <Card><CardContent className="pt-6"><form onSubmit={submit} className="space-y-5">
            ...
            <div className="grid gap-4 md:grid-cols-2"><div><Label>{t('Preferred script provider')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.ai_provider_id} onChange={(e)=>form.setData('ai_provider_id', e.target.value)}><option value="">{t('Default provider')}</option>{scriptProviders.map((x:any)=><option key={x.id} value={x.id}>{x.name} ({t(x.provider_category === 'grounded_text' ? 'News with search and sources' : x.provider_category === 'text' ? 'Scripts / Text' : x.provider_category === 'local' ? 'Local / Offline' : 'Other')})</option>)}</select><p className="mt-1 text-xs text-slate-500">{t('Use Gemini Grounded for current news with sources')} · {t('Use Groq for fast script generation from prepared context')}</p></div><div><Label>{t('Language')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.language_id} onChange={(e)=>form.setData('language_id', e.target.value)}><option value="">{t('Any')}</option>{languages.map((x:any)=><option key={x.id} value={x.id}>{x.name} ({x.code})</option>)}</select></div><div><Label>{t('Default prompt profile')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.default_prompt_profile_id} onChange={(e)=>form.setData('default_prompt_profile_id', e.target.value)}><option value="">{t('Any')}</option>{promptProfiles.map((x:any)=><option key={x.id} value={x.id}>{x.name}</option>)}</select></div></div>
            ...
            <div className="space-y-3 rounded-lg border p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2"><div><Label>{t('Prompt language')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.prompt_language} onChange={(e)=>form.setData('prompt_language', e.target.value)}>{promptLanguages.map((x:string)=><option key={x} value={x}>{x}</option>)}</select></div><div><Label>{t('Output mode')}</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={form.data.output_mode} onChange={(e)=>form.setData('output_mode', e.target.value)}>{outputModes.map((x:string)=><option key={x} value={x}>{t(x === 'final_plain_script' ? 'Simple final script' : x === 'plain_script' ? 'Plain final script' : 'Advanced structured script')} {x === 'structured_script' ? `(${t('Advanced / debug')})` : ''}</option>)}</select></div></div>
                <p className="text-xs text-slate-600">{form.data.output_mode === 'final_plain_script' ? t('This mode is recommended for automation') : form.data.output_mode === 'structured_script' ? t('Structured mode is only for advanced debugging') : t('The AI response can be saved directly as a script')}</p>
