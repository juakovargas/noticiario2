...
interface Location { id:number; parent_id:number|null; name:string; slug:string; type:string; country_code:string|null; timezone:string|null; default_language_id:number|null; latitude:number|null; longitude:number|null; map_zoom:number|null; marker_color:string|null; marker_label:string|null; show_on_map:boolean; is_active:boolean; sort_order:number; }
interface Props { location:Location; parents:Option[]; types:string[]; languages:Option[]; }

export default function Edit({ location, parents, types, languages }: Props): JSX.Element {
    const { t } = useTranslations();
    const { data, setData, put, processing, errors } = useForm({ parent_id: location.parent_id?.toString() || '', name: location.name, slug: location.slug, type: location.type, country_code: location.country_code || '', timezone: location.timezone || '', default_language_id: location.default_language_id?.toString() || '', latitude: location.latitude?.toString() || '', longitude: location.longitude?.toString() || '', map_zoom: location.map_zoom?.toString() || '', marker_color: location.marker_color || '', marker_label: location.marker_label || '', show_on_map: location.show_on_map, is_active: location.is_active, sort_order: location.sort_order });
    const submit = (e: React.FormEvent) => { e.preventDefault(); put(route('editor.locations.update', location.id)); };

    return <EditorLayout><Head title={t('Edit Location')} /><AdminPageHeader title={t('Edit Location')} description={t('Update location details.')} />
