...
interface Location { id:number; parent_id:number|null; name:string; slug:string; type:string; country_code:string|null; timezone:string|null; default_language_id:number|null; is_active:boolean; sort_order:number; }
...
export default function Edit({ location, parents, types, languages }: Props): JSX.Element {
    const { data, setData, put, processing, errors } = useForm({ parent_id: location.parent_id?.toString() || '', name: location.name, slug: location.slug, type: location.type, country_code: location.country_code || '', timezone: location.timezone || '', default_language_id: location.default_language_id?.toString() || '', is_active: location.is_active, sort_order: location.sort_order });
    ...
    return <EditorLayout><Head title="Edit Location" /><AdminPageHeader title="Edit Location" description="Update location details." />
        <Card><CardContent className="pt-6"><form onSubmit={submit} className="space-y-4">
            ...
            <div><Label>Default language</Label><select className="w-full rounded-md border border-slate-300 px-3 py-2" value={data.default_language_id} onChange={(e)=>setData('default_language_id', e.target.value)}><option value="">Not configured</option>{languages.map((l)=><option key={l.id} value={l.id}>{l.flag_emoji} {l.native_name || l.name} ({l.code})</option>)}</select></div>
