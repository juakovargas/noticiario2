...
export default function Index({ locations }: Props): JSX.Element {
    ...
    return <EditorLayout><Head title={t('Locations')} /><AdminPageHeader title={t('Locations')} description="Manage geographic scopes for news and editions." actionLabel={t('Create')} actionHref={route('editor.locations.create')} />
        <Card><CardContent className="overflow-x-auto pt-6"><table className="w-full min-w-[720px] text-left text-sm"><thead><tr className="border-b border-slate-200 text-slate-500"><th className="px-2 pb-3">{t('Name')}</th><th className="px-2 pb-3">{t('Type')}</th><th className="px-2 pb-3">Country</th><th className="px-2 pb-3">{t('Status')}</th><th className="px-2 pb-3">Sort</th><th className="px-2 pb-3 text-right">{t('Actions')}</th></tr></thead><tbody>{locations.data.length ? locations.data.map((item)=><tr key={item.id} className="border-b border-slate-100"><td className="px-2 py-3 font-medium">{item.name}</td><td className="px-2 py-3">{item.type}</td><td className="px-2 py-3">{countryCodeToFlagEmoji(item.country_code)} {item.country_code || '-'}</td><td className="px-2 py-3">{item.is_active ? t('Active') : t('Inactive')}</td><td className="px-2 py-3">{item.sort_order}</td><td className="px-2 py-3"><div className="flex justify-end gap-2"><Button asChild size="sm" variant="secondary"><Link href={route('editor.locations.edit', item.id)}>{t('Edit')}</Link></Button><Button size="sm" variant="destructive" onClick={()=>destroy(item.id)}>{t('Delete')}</Button></div></td></tr>) : <tr><td colSpan={6} className="px-2 py-6 text-center text-slate-500">No locations yet.</td></tr>}</tbody></table><Pagination links={locations.links} /></CardContent></Card></EditorLayout>;
}
