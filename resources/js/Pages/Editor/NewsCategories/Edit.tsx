...
interface Category { id:number; parent_id:number|null; name:string; slug:string; description:string|null; name_es?:string|null; description_es?:string|null; color:string|null; icon:string|null; is_active:boolean; sort_order:number; }
interface Props { newsCategory:Category; parents:Option[]; }
export default function Edit({ newsCategory, parents }: Props): JSX.Element {
 const { data, setData, put, processing } = useForm({ parent_id:newsCategory.parent_id?.toString() || '', name:newsCategory.name, slug:newsCategory.slug, description:newsCategory.description || '', name_es:newsCategory.name_es || '', description_es:newsCategory.description_es || '', color:newsCategory.color || '#94a3b8', icon:newsCategory.icon || '', is_active:newsCategory.is_active, sort_order:newsCategory.sort_order });
