...
interface Item { id:number; title:string|null; source_name:string|null; source_url:string|null; source_type:string; verification_status:string; trust_level:number|null; checked_by:{ id:number; name:string; email:string | null; avatar_url?:string|null; initials?:string|null }|null; checked_at:string|null; archived_at:string|null; script:{id:number;title:string}|null; news_item:{id:number;title:string}|null; script_review_item:{id:number;title:string|null}|null; }
interface Props { sourceReferences:{ data:Item[]; links:Array<{url:string|null;label:string;active:boolean}>}; filters: Record<string,string>; verificationStatuses:string[]; sourceTypes:string[]; }
...
export default function Index({ sourceReferences, filters, verificationStatuses, sourceTypes }: Props): JSX.Element {
    ...
    return <EditorLayout>
        ...
        <Card className="mb-4"><CardContent className="pt-6"><form className="grid gap-3 md:grid-cols-6" onSubmit={submit}>
            ...
            <select className="rounded border px-3 py-2 text-sm" value={form.source_type ?? ''} onChange={(e)=>setForm((prev)=>({...prev,source_type:e.target.value}))}><option value="">{t('Source type')}</option>{sourceTypes.map((item)=><option key={item} value={item}>{item}</option>)}</select>
        ...
        </tr></thead><tbody>{sourceReferences.data.length ? sourceReferences.data.map((item)=><tr key={item.id} className="border-b border-slate-100"><td className="px-2 py-3"><p className="font-medium">{item.title || item.source_name || '-'}</p><p className="text-xs text-slate-500">{item.source_name || '-'}</p></td><td className="px-2 py-3">{item.source_url ? <a href={item.source_url} target="_blank" rel="noreferrer" className="text-cyan-700 underline">{t('Open source')}</a> : '-'}</td><td className="px-2 py-3">{item.source_type}</td><td className="px-2 py-3"><Badge className={badgeClass[item.verification_status] ?? badgeClass.pending}>{item.verification_status === 'not_required' ? t('Not required') : t(item.verification_status.charAt(0).toUpperCase()+item.verification_status.slice(1))}</Badge></td><td className="px-2 py-3">{item.trust_level ?? '-'}</td><td className="px-2 py-3 text-xs">{item.script ? `${t('Linked script')}: ${item.script.title}` : '-'}<br />{item.news_item ? `${t('Linked news item')}: ${item.news_item.title}` : ''}<br />{item.script_review_item ? `${t('Linked review item')}: #${item.script_review_item.id}` : ''}</td><td className="px-2 py-3">{item.checked_by ? <UserIdentity user={item.checked_by} subtitle={item.checked_by.email} avatarSize="xs" /> : '-'}</td><td className="px-2 py-3">{formatDateTime(item.checked_at)}</td><td className="px-2 py-3 text-right"><div className="flex justify-end gap-2"><Button asChild size="sm" variant="outline"><Link href={route('editor.source-references.show', item.id)}>{t('View')}</Link></Button>{(item as any).archived_at ? <Button size="sm" variant="outline" onClick={()=>router.post(route('editor.source-references.restore', item.id))}>{t('Restore')}</Button> : <Button size="sm" variant="outline" onClick={()=>router.post(route('editor.source-references.archive', item.id))}>{t('Archive')}</Button>}</div></td></tr>) : <tr><td colSpan={9} className="px-2 py-6 text-center text-slate-500">{t('No source references found')}</td></tr>}</tbody></table><Pagination links={sourceReferences.links} /></CardContent></Card>
    </EditorLayout>;
}
