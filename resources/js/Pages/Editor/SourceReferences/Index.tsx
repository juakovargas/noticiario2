...
interface Item { id:number; title:string|null; source_name:string|null; source_url:string|null; source_type:string; verification_status:string; trust_level:number|null; checked_by:string|null; checked_at:string|null; script:{id:number;title:string}|null; news_item:{id:number;title:string}|null; script_review_item:{id:number;title:string|null}|null; }
...
export default function Index({ sourceReferences, filters, verificationStatuses, sourceTypes }: Props): JSX.Element {
    ...
    return <EditorLayout>
        ...
        </tr></thead><tbody>{sourceReferences.data.length ? sourceReferences.data.map((item)=><tr key={item.id} className="border-b border-slate-100"><td className="px-2 py-3"><p className="font-medium">{item.title || item.source_name || '-'}</p><p className="text-xs text-slate-500">{item.source_name || '-'}</p></td><td className="px-2 py-3">{item.source_url ? <a href={item.source_url} target="_blank" rel="noreferrer" className="text-cyan-700 underline">{t('Open source')}</a> : '-'}</td><td className="px-2 py-3">{item.source_type}</td><td className="px-2 py-3"><Badge className={badgeClass[item.verification_status] ?? badgeClass.pending}>{item.verification_status === 'not_required' ? t('Not required') : t(item.verification_status.charAt(0).toUpperCase()+item.verification_status.slice(1))}</Badge></td><td className="px-2 py-3">{item.trust_level ?? '-'}</td><td className="px-2 py-3 text-xs">{item.script ? `${t('Linked script')}: ${item.script.title}` : '-'}<br />{item.news_item ? `${t('Linked news item')}: ${item.news_item.title}` : ''}<br />{item.script_review_item ? `${t('Linked review item')}: #${item.script_review_item.id}` : ''}</td><td className="px-2 py-3">{item.checked_by ?? '-'}</td><td className="px-2 py-3">{formatDateTime(item.checked_at)}</td><td className="px-2 py-3 text-right"><Button asChild size="sm" variant="outline"><Link href={route('editor.source-references.show', item.id)}>{t('View')}</Link></Button></td></tr>) : <tr><td colSpan={9} className="px-2 py-6 text-center text-slate-500">{t('No source references found')}</td></tr>}</tbody></table><Pagination links={sourceReferences.links} /></CardContent></Card>
    </EditorLayout>;
}
