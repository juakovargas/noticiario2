...
return <Wrap><Head title={t('Message')} /><Card><CardHeader><CardTitle>{message.subject}</CardTitle></CardHeader><CardContent><p className='text-xs'>{t('Sender')}: {message.sender?.name ?? '-'} · {t('Sent to')}: {message.recipient?.name ?? '-'} · {formatDateTime(message.created_at)}</p><p className='text-xs'>{t('Recipient reason')}: {recipientReason ?? '-'}</p><p className='mt-4 whitespace-pre-wrap'>{message.body}</p>{relatedLink?<><a className='mt-4 inline-block underline' href={relatedLink.url}>{t('Open related item')}</a><p className='text-xs mt-1'>{t('This link opens in your current panel')}</p></>:<p className='mt-4 text-xs'>{t('No accessible related link')}</p>}<Link className='mt-4 block underline' href={route('messages.index')}>{t('Open messages')}</Link></CardContent></Card></Wrap> }
